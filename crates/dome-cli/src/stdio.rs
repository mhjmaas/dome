...
use dome_proto::{frame, WatchEvent};
use dome_vm::Sandbox;
...
pub(crate) fn run_stdio(prepared: &PreparedVm) -> Result<i32> {
    ...
    let (vm_fd, proxy_handle, _audit_handle) = if let Some(ref proxy_config) = prepared.proxy_config
    {
        let (vm_fd, host_fd) = dome_proxy::create_socketpair()?;
        let audit = vm::start_audit_writer(
...
fn handle_checkpoint(
    ...
    if let Err(msg) = dome_vm::validate_checkpoint_name(name) {
