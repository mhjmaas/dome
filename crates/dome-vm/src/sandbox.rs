...
#[cfg(target_os = "macos")]
use dome_darwin::network::FileHandleNetworkAttachment;
#[cfg(target_os = "macos")]
...
#[cfg(target_os = "linux")]
use dome_linux::network::FileHandleNetworkAttachment;
#[cfg(target_os = "linux")]
...
impl Sandbox {
                ...
                Err(_) => {
                    bail!(
                        "guest does not support directory mounts. \
                         Run `dome upgrade` and recreate the checkpoint to enable --mount."
                    );
                }
            };
            if !resp.ok {
    ...
    pub fn download(
        ...
        on_progress: impl Fn(dome_proto::DownloadProgress),
    ) -> Result<()> {
        let stream = self.connect_vsock()?;
        let mut writer = stream.try_clone()?;
        let mut reader = stream;
        ...
        let req = dome_proto::DownloadRequest {
            url: url.to_string(),
            path: path.to_string(),
            extract,
            strip_components,
        ...
        loop {
            match frame::read_frame(&mut reader)? {
                Some((frame::DOWNLOAD_PROGRESS, payload)) => {
                    if let Ok(progress) =
                        serde_json::from_slice::<dome_proto::DownloadProgress>(&payload)
                    {
    ...
    pub fn start_port_forwarding(&self, forwards: &[PortMapping]) -> Result<PortForwardHandle> {
        ...
        for mapping in forwards {
            ...
            eprintln!(
                "dome: forwarding 127.0.0.1:{} -> guest:{}",
